...
contract ArcBounty is ArcDefi {
    constructor(address _eurc) ArcDefi(_eurc) {}
    struct B { address poster; string desc; uint256 reward; bool open; address winner; }
    struct Sub { uint256 bountyId; address worker; string proof; bool approved; }
    B[] public bounties; Sub[] public subs;
    mapping(uint256 => uint256[]) private subsOf;
    mapping(address => uint256[]) private postedBy;
    event Posted(uint256 indexed id, address indexed poster, uint256 reward);
    event Submitted(uint256 indexed bountyId, uint256 indexed subId, address worker);
    event Approved(uint256 indexed bountyId, address winner, uint256 reward);
    function post(string calldata desc) external payable returns (uint256 id) {
        require(msg.value > 0, "no reward");
        id = bounties.length; bounties.push(B(msg.sender, desc, msg.value, true, address(0)));
        postedBy[msg.sender].push(id); emit Posted(id, msg.sender, msg.value);
    }
    function submit(uint256 bountyId, string calldata proof) external returns (uint256 subId) {
        require(bounties[bountyId].open, "closed");
        subId = subs.length; subs.push(Sub(bountyId, msg.sender, proof, false));
        subsOf[bountyId].push(subId); emit Submitted(bountyId, subId, msg.sender);
    }
    function approve(uint256 subId) external {
        Sub storage s = subs[subId]; B storage b = bounties[s.bountyId];
        require(msg.sender == b.poster && b.open, "no");
        b.open = false; b.winner = s.worker; s.approved = true;
        (bool ok,) = payable(s.worker).call{value: b.reward}(""); require(ok, "fail");
        emit Approved(s.bountyId, s.worker, b.reward);
    }
    function get(uint256 id) external view returns (B memory) { return bounties[id]; }
    function getSub(uint256 id) external view returns (Sub memory) { return subs[id]; }
    function getSubs(uint256 bountyId) external view returns (uint256[] memory) { return subsOf[bountyId]; }
    function getPosted(address u) external view returns (uint256[] memory) { return postedBy[u]; }
    function total() external view returns (uint256) { return bounties.length; }
}
